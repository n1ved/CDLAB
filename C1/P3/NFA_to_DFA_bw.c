...
void sort_nfa(){
    for(int st =0 ; st<n_states ; st++){
        for(int sy=0 ; sy<n_symbol ; sy++){
            ...
            if(count){
                bubble_sort(count , nfa_trans[st][sy].states);
            }
        }
    }
}
    ...
    while(pq_rear >= pq_front){
        ...
        for(int sy = 0 ; sy<n_symbol ; sy++){
            ...
            if(new_state_count ){
                ...
                if (!dup_in_dfa_map(new_state_count, new_state))
                ...
                for(int i=0 ; i<new_state_count ; i++){
                    ...
                    if (!dup_in_dfa_map(new_state_count, new_state))
                        dfa_map[dfa_map_ptr-1].states[i] = new_state[i];
                }
                if (!dup_in_dfa_map(new_state_count, new_state))
                    enqueue(dfa_map_ptr-1);
            }
        }
    }
}
